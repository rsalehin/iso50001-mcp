...
def compare_current_vs_baseline(facility_id: str, period: str) -> dict:
    ...
    return {
        ...
        "status": status,
        "performance_summary": f"{abs(delta_pct)}% {'below' if delta_kwh < 0 else 'above'} baseline",
    }
...
def assess_measure_effectiveness(measure_id: str) -> dict:
    ...
    if measure["actual_saving_kwh"] is not None:
        ...
        result["assessment_summary"] = (
            f"Achieved {measure['actual_saving_kwh']:,.0f} kWh savings vs "
            f"{measure['projected_saving_kwh']:,.0f} kWh projected "
            f"({abs(variance_pct)}% {'above' if variance >= 0 else 'below'} target)."
        )
    else:
        result["assessment_summary"] = "Measure in progress - actual savings not yet recorded."
    return result

# TOOL 4 - SQL + LLM (only tool that calls Claude)
def draft_management_review_section(facility_id: str, period: str) -> dict:
    ...
    measures_text = "\n".join([
        f"- {m['name']} ({m['implementation_date']}): projected {m['projected_saving_kwh']:,.0f} kWh, "
        f"actual {m['actual_saving_kwh']:,.0f} kWh, status: {m['status']}"
        if m["actual_saving_kwh"] else
        f"- {m['name']} ({m['implementation_date']}): projected {m['projected_saving_kwh']:,.0f} kWh, status: {m['status']}"
        for m in measures
    ])
    prompt = f"""You are an ISO 50001 compliance specialist. Draft the Energy Performance section for a management review document based on the following verified data.
